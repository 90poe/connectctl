# connectctl installation

The contains details of how to install and uninstall connectl

## Requirements

connectctl runs on Linux, Windows and MacOS.

## Download the binary

The connectctl binary can be downloaded from the [releases section](https://github.com/90poe/connectctl/releases). Its advised that you make the binary available in your PATH.

## Future work

We'll be supplying an install script and support for various package managers in the future.

